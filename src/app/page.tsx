...
import { FormData, ChatMessage, AnalysisResult } from "@/lib/types";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(
      words,
      { y: "100%", opacity: 0 },
      { y: "0%", opacity: 1, duration: 1.4, ease: "power4.out", stagger: 0.06, delay: 0.3 }
    );
    gsap.fromTo(
      ".hero-search-bar",
  ...
  // Total pages = form steps + chat page + result pages (5)
  const formPageCount = onboardingSteps.length;
  const chatPageIndex = formPageCount;
  const resultStartIndex = chatPageIndex + 1;
  const resultPageCount = analysis ? 5 : 0;
  const totalPages = formPageCount + 1 + resultPageCount;

  const handleFormNext = useCallback(() => {
    const currentFormStep = currentPage;
    if (currentFormStep < formPageCount - 1) {
      setCurrentPage((prev) => prev + 1);
    } else {
      // Last form step - move to chat page and start guidance
      const priorityScore = calculatePriorityScore(formData);
      const updatedFormData = { ...formData, priority_score: priorityScore };
      setFormData(updatedFormData);
      setCurrentPage(chatPageIndex);
  ...
  }, [currentPage, formPageCount, formData, chatPageIndex, guidanceStarted]);
      ...
      setMessages([
        {
          id: crypto.randomUUID(),
          role: "assistant",
          content: "I apologize, but I encountered an issue connecting to the AI service. Please try again.",
          timestamp: Date.now(),
        },
      ]);
    } finally {
  ...
  const handleFinishGuidance = async () => {
    if (!analysis) {
      ...
      try {
        const response = await fetch("/api/chat", {
          ...
          body: JSON.stringify({
            messages: [
              ...
              {
                role: "user",
                content: "Please provide your final comprehensive analysis with the <analysis> JSON block including scores, key findings, recommendations, risk level, confidence, and breakdown data.",
              },
            ],
            formData,
          }),
        ...
        if (result.message) {
          setMessages((prev) => [
            ...prev,
            { id: crypto.randomUUID(), role: "assistant", content: result.message, timestamp: Date.now() },
          ]);
        }
      } catch {
        setAnalysis(buildFallbackAnalysis());
    ...
    // Navigate to results (first result page)
    setCurrentPage(resultStartIndex);
  };
  ...
  const buildFallbackAnalysis = (): AnalysisResult => ({
    ...
    scores: { relevance: 75, completeness: 68, actionability: 82, clarity: 79, depth: 71 },
    recommendations: [
  ...
  if (!started) {
    return (
      <div className="relative w-full min-h-screen flex flex-col justify-end pb-24 md:pb-32 px-6 md:px-12 overflow-hidden">
          ...
          <div className="max-w-4xl">
            ...
            <div className="hero-search-bar opacity-0 mt-16 bg-white/10 backdrop-blur-md border border-white/20 rounded-[2px] px-8 py-6 flex flex-col md:flex-row items-center gap-6 max-w-5xl">
              <div className="w-full md:flex-1 group">
                <span className="block text-white/50 text-[10px] uppercase tracking-widest mb-2">Domain</span>
              ...
              <div className="w-full md:flex-1 group">
                <span className="block text-white/50 text-[10px] uppercase tracking-widest mb-2">Format</span>
                <div className="border-b border-white/20 pb-2 group-hover:border-white transition-colors duration-300">
                  <span className="text-white text-sm font-light">Interactive Flipbook</span>
                </div>
              </div>
              <div className="w-full md:flex-1 group">
                <span className="block text-white/50 text-[10px] uppercase tracking-widest mb-2">AI Engine</span>
                <div className="border-b border-white/20 pb-2 group-hover:border-white transition-colors duration-300">
              ...
              <div className="w-full md:w-auto mt-4 md:mt-0">
                <button
                  onClick={() => { setStarted(true); setCurrentPage(0); }}
  ...
  // ==================== BUILD ALL FLIPBOOK PAGES ====================
  const allPages: React.ReactNode[] = [];

  // --- Form steps (pages 0 to formPageCount-1) ---
  ...
  // --- Chat page (page at chatPageIndex) ---
  allPages.push(
  ...
  // --- Result pages (if analysis is available) ---
  if (analysis) {
    // Result cover
    allPages.push(
      <div key="result-cover" className="h-full flex flex-col justify-center items-center text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          <span className="text-[10px] uppercase tracking-[0.3em] text-stone-400 block mb-4">Analysis Complete</span>
          <h1 className="text-3xl md:text-4xl text-stone-900 tracking-tight mb-4" style={{ fontFamily: "'Playfair Display', serif" }}>
            {analysis.title}
          </h1>
          ...
          <div className="flex items-center justify-center gap-8">
            <D3GaugeChart value={Math.round(analysis.confidence * 100)} label="Confidence" />
          </div>
        </motion.div>
      </div>
    );
    ...
    allPages.push(
      <div key="findings" className="h-full flex flex-col">
        <div className="mb-6">
          <span className="text-[10px] uppercase tracking-widest text-stone-400 block mb-2">Key Findings</span>
          <h2 className="text-2xl text-stone-900 tracking-tight" style={{ fontFamily: "'Playfair Display', serif" }}>What We Discovered</h2>
        </div>
        <div className="flex-1 space-y-3">
          {analysis.key_findings.map((finding, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.15 }} className="flex gap-3 p-4 bg-stone-50 rounded-sm">
              <span className="text-xs text-stone-400 font-medium mt-0.5">{String(i + 1).padStart(2, "0")}</span>
              <p className="text-sm text-stone-700 font-light">{finding}</p>
            </motion.div>
          ))}
        </div>
        <div className="mt-6"><RiskIndicator level={analysis.risk_level} /></div>
      </div>
    );
    ...
    allPages.push(
      <div key="scores" className="h-full flex flex-col">
        <div className="mb-4">
          <span className="text-[10px] uppercase tracking-widest text-stone-400 block mb-2">Scores</span>
          <h2 className="text-2xl text-stone-900 tracking-tight" style={{ fontFamily: "'Playfair Display', serif" }}>Performance Metrics</h2>
        </div>
    ...
    allPages.push(
      <div key="breakdown" className="h-full flex flex-col">
        <div className="mb-4">
          <span className="text-[10px] uppercase tracking-widest text-stone-400 block mb-2">Deep Dive</span>
          <h2 className="text-2xl text-stone-900 tracking-tight" style={{ fontFamily: "'Playfair Display', serif" }}>Detailed Breakdown</h2>
        </div>
    ...
    allPages.push(
      <div key="recommendations" className="h-full flex flex-col">
        <div className="mb-6">
          <span className="text-[10px] uppercase tracking-widest text-stone-400 block mb-2">Recommendations</span>
          <h2 className="text-2xl text-stone-900 tracking-tight" style={{ fontFamily: "'Playfair Display', serif" }}>Next Steps</h2>
        </div>
        <div className="flex-1 space-y-4">
          {analysis.recommendations.map((rec, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.2 }} className="flex gap-4 items-start p-4 border border-stone-100 rounded-sm hover:border-stone-300 transition-colors">
        ...
        <div className="mt-8 pt-6 border-t border-stone-100 flex items-center justify-between">
          <button onClick={() => setCurrentPage(chatPageIndex)} className="btn-outline">Back to Chat</button>
          <button
            onClick={() => {
              setStarted(false);
              setFormData({});
  ...
  return (
    <div className="min-h-screen bg-[#f9f8f6] flex items-center justify-center px-4 py-12">
      ...
      <div className="fixed top-6 right-6 z-50">
        <span className="text-xs uppercase tracking-[0.3em] text-stone-400 font-medium">BRANCED</span>
      </div>

      <Flipbook currentPage={currentPage} onPageChange={setCurrentPage}>
        {allPages}
      </Flipbook>
    </div>
  );
